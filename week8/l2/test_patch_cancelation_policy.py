...
from patch_cancelation_policy import (
    validate_conditions,
    ensure_conditions,
    pair_conditions,
...
class TestGetCancellationPolicy(unittest.TestCase):
    @patch('patch_cancelation_policy.datetime')
    def test_with_now_equal_to_booking_start_should_raise_error(self, now_mock):
        now_mock.now.return_value = datetime(year=2020, month=4, day=27, hour=12, minute=50, second=26)
        conditions = [{'percent': 50}]
        booking_start = now_mock.now.return_value
        exc = None
    ...
    @patch('patch_cancelation_policy.datetime')
    def test_with_now_later_than_booking_start_should_raise_error(self, now_mock):
        conditions = [{'percent': 50}]
        now_mock.now.return_value = datetime(year=2020, month=4, day=27, hour=12, minute=50, second=26)
        booking_start = now_mock.now.return_value - timedelta(hours=1)
        exc = None
    ...
    @patch('patch_cancelation_policy.datetime')
    def test_cancellation_fee_with_only_one_condition(self, now_mock):
        conditions = [{'percent': 50}]
        now_mock.now.return_value = datetime(year=2020, month=2, day=5, hour=12, minute=50, second=26)
        price = 100
        booking_start = datetime.now()
    ...
    @patch('patch_cancelation_policy.datetime')
    def test_cancellation_fee_with_several_conditions_and_now_in_them(self, now_mock):
        conditions = [
            {'hours': 24, 'percent': 10},
            {'hours': 12, 'percent': 50},
            {'hours': 6, 'percent': 80},
        ...
        now_mock.now.return_value = datetime(year=2020, month=4, day=28, hour=12, minute=50, second=26)

        price = 100
        booking_start = now_mock.now.return_value + timedelta(hours=10)
        result = get_cancellation_policy(conditions, price, booking_start)

        self.assertEqual(result, 80.0)

    @patch('patch_cancelation_policy.datetime')
    def test_cancellation_fee_with_several_conditions_and_now_in_them_and_decimal_percent(self, now_mock):
        conditions = [
            {'hours': 24, 'percent': 10},
            {'hours': 12, 'percent': 50},
